...
export default function WordButtons({ setShowMeaning, nextWord, prevWord, randomWord, resetWord} : Props) {
  return (
    <div
      ...
      <button 
        onClick={nextWord}
        className="px-4 py-2 rounded-2xl shadow-md bg-green-500 text-white hover:bg-green-600 transition">次へ →</button>
      <button 
        onClick={randomWord}
        className="px-4 py-2 rounded-2xl shadow-md bg-yellow-500 text-white hover:bg-yellow-600 transition">🎲 ランダム</button>
      <button 
        onClick={resetWord}
